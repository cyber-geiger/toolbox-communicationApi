...
public class DemoPluginStageProvider {
  ...
  private Node[] getInitialNodes() {
    ...
    threatMap
        .put("80efffaf-98a1-4e0a-8f5e-gr89388361ee", new String[]{"External environment threats"});
    ...
    threatMap
        .put("80efffaf-98a1-4e0a-8f5e-th89388364ds", new String[]{"DDoS", "Denial of service"});
    threatMap
        .put("80efffaf-98a1-4e0a-8f5e-th89388365it", new String[]{"Identity theft", "Data breach"});
    threatMap
        .put("80efffaf-98a1-4e0a-8f5e-th89388366ph", new String[]{"Physical", "Physical threats"});
      ...
      Node threat = new NodeImpl(":Global:threats:" + e.getKey(), Visibility.RED,
          new NodeValue[]{
              ...
              new NodeValueImpl("GEIGER_threat",
                  e.getValue().length > 1 ? e.getValue()[1] : e.getValue()[0])
          }, null
      );
      l.add(threat);
    }
    ...
    try {
      for (Map.Entry<String, String[]> e : childMap.entrySet()) {
        ...
        for (int i = 1; i < a.length; i++) {
          ...
          node.addValue(new NodeValueImpl(getThreatUuid(), threatValue));
        }
        l.add(node);
      }
    } catch (StorageException sx) {
      System.out.println("OOPS.... got unexpected storage exception");
      sx.printStackTrace();
    }
