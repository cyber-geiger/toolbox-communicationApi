...
class GeigerCommunicator {
  ...
  Future<void> sendMessage(PluginInformation target, Message message) async {
    final socket =
    ...
    final bs = ByteStream(socket);
    final uid = await SerializerHelper.readLong(bs)
        .timeout(Duration(milliseconds: timeout), onTimeout: () {
      throw const SocketException('iOS connection refused detected.');
    });
    SerializerHelper.castTest('Message', responseUID, uid, 1);
    socket.destroy();
  }
}
