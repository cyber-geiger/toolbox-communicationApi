...
Future <Serializer?> readObject(final ByteStream inStream) async {
  if(SerializerHelper.byteArrayToInt(await inStream.peekBytes(SerializerHelper.longSize))==StorableString.serialVersionUID ) {
    return StorableString.fromByteArrayStream(inStream);
  } else if(SerializerHelper.byteArrayToInt(await inStream.peekBytes(SerializerHelper.longSize))==StorableHashMap.serialVersionUID ) {
    return StorableHashMap.fromByteArrayStream(inStream,StorableHashMap());
  } else if(SerializerHelper.byteArrayToInt(await inStream.peekBytes(SerializerHelper.longSize))==PluginInformation.serialVersionUID ) {
    return PluginInformation.fromByteArrayStream(inStream);
  } else if(SerializerHelper.byteArrayToInt(await inStream.peekBytes(SerializerHelper.longSize))==ParameterList.serialVersionUID ) {
    return ParameterList.fromByteArrayStream(inStream);
  } else if(SerializerHelper.byteArrayToInt(await inStream.peekBytes(SerializerHelper.longSize))==MenuItem.serialVersionUID ) {
    return MenuItem.fromByteArrayStream(inStream);
  } else if(SerializerHelper.byteArrayToInt(await inStream.peekBytes(SerializerHelper.longSize))==GeigerUrl.serialVersionUID ) {
