...
public class TestPluginInformation {
  ...
  String[] executables = new String[]{"thisApplication.jar", "./thisAccplication",
      "./../path/to/thisApplication", "C:/path to/this/application", "thisApplication.apk"};
  CommunicationSecret[] secrets = new CommunicationSecret[]{new CommunicationSecret(),
    ...
    for (int port : ports) {
      for (String executable : executables) {
        for (CommunicationSecret secret : secrets) {
          ...
          Assert.assertEquals("checking Executable", executable, info.getExecutable());
          Assert.assertEquals("checking port", port, info.getPort());
          Assert.assertArrayEquals("checking secret", secret.getSecret(),
              info.getSecret().getSecret());
        }
      }
    }
  }
